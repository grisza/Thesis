\begin{tikzpicture} [		
	...
	\begin{scope}
		...
		% \node [right=of loop3, xshift=3cm] (nullCorS) {};
		% \node [action, below=of nullCorS] (corS) {$cor_s = cor_{s-1} - diag(\frac{1}{a_{x,th}},\frac{1}{a_{y,th}},\frac{1}{a_{z,th}},\frac{1}{g_{x,th}},\frac{1}{g_{y,th}},\frac{1}{g_{z,th}}) \left(\begin{bmatrix}A_0 \\ G_0\end{bmatrix} - \begin{bmatrix}\overline{A}\\ \overline{G}\end{bmatrix}_s\right)$};\\
	...
	\begin{scope} [every path/.style=line]
		...
		% \path  (loop3)  --++  (-3,0) node [near start, above] {Tak} -| (cor1) --++ (0, -1) node(temp) {} -|  (incS.north);
		% \path  (loop3)  --++  (3,0) node [near start, above] {Nie} -| (corS) --++  (0, -0.83) node(temp) {} -|  (incS.north);
		                 
		% \path  (incS.east)  --++  (5, 0) node {} --++ (5, 17) node {} |-  (nullResetCounter.east);
	\end{scope}			
\end{tikzpicture}