...
\subsection{Systemy optyczne śledzenia ruchu}
...
%Markerami wykorzystywanymi w~systemach śledzenia mogą być zarówno punktowe źródła światła (tzw. markery aktywne) jak również małe obiekty odbijające światło zewnętrzne (tzw. markery pasywne). 
...
Większość oferowanych w~tech chwili komercyjnych optycznych systemów śledzenia ruchu pracuje standardowo z~częstotliwością 100 Hz -- 160 Hz, jednak możliwe jest także używanie ich w~trybie 500Hz i~więcej. Zwiększenie częstotliwości pomiarów wiąże się zazwyczaj z~obniżeniem ich dokładności.
...
%W tym samym roku Chris Sweeney et al. \cite{Sweeney2015} opublikowali artykuł dotyczący wykorzystania czujników inercyjnych do określania obrotów w~środowiskach rozszerzonej rzeczywistości z~wykorzystaniem 
...
W zależności od specyfiki wykonywanego ruchu, celu w jakim jest on śledzony oraz ilości i jakości urządzeń pomiarowych model kinematyczny może być zdefiniowany dla przestrzeni dwu-- lub trójwymiarowej.\\[2cm] ([TODO] co oznacza te 2cm? może jakieś referencje dla 2D i jakieś dla 3D?)
	...
	Kolejnym krokiem po wyznaczeniu położenia poszczególnych markerów, na podstawie obrazów z kamer, jest interpretacja tych danych w taki sposób, aby uzyskać opis stanu stawów, które są reprezentowane przez pojedyncze punkty ([TODO] punkt nie ma orientacji a staw ma!!). Interpretacja ta odbywa się bądź to na podstawie obliczeń matematycznych wynikających wprost z geometrii wyznaczonych punktów ([TODO] co to jest geometria punktu?) lub na podstawie przyjętego modelu biomechanicznego ciała ludzkiego. Przykładem wyznaczenia położenia stawu może być wyznaczenie położenia kości miedniczej (\emph{ang. pelvis}). Rysunek \ref{fig:literature:vicon:pelvisPlacement} pokazuje położenie markerów wykorzystywanych do wyznaczenia położenia stawu reprezentującego kość miedniczą. Według dokumentacji \cite{ViconModelingInstruction} położenie kości miedniczej jest szacowane jako średnia arytmetyczna punktów RASI i LASI (rys. \ref{fig:literature:vicon:pelvisPlacementA}). Jeśli jeden z tych dwóch punktów jest niewidoczny, wówczas położenie stawu reprezentującego kość miedniczą jest wyznaczane jako wierzchołek trójkąta prostokątnego, którego przeciwprostokątną wyznaczają punkty SACR oraz jeden z widocznych RASI lub LASI.
	...
	\begin{subequations}
		\begin{align}
			X & = C * \cos(\theta) * \sin(\beta) - (h + m) * \cos(\beta) \label{eq:hipEquation:Xcoordinate} \\
			Y & = -(C * \sin(\theta) - a) \label{eq:hipEquation:Ycoordinate}                                \\
			Z & = -C * \cos(\theta)*\cos(\beta) - (h + m) * \sin(\beta) \label{eq:hipEquation:Zcoordinate}  
		\end{align}
		\label{eq:hipEquation:XYZcoordinates}
	\end{subequations}
	
	Przyjmując, że $l_l$ i $l_p$ to długości kości udowych obu nóg, współczynnik $C$ zdefiniowany jest wg wzoru \eqref{eq:hipEquation:Ccoeficient}
	
	\begin{equation}
		\centering
		C = \frac{l_l + l_p}{2} * 0.115 - 15.3
		\label{eq:hipEquation:Ccoeficient}
	\end{equation}
	
	Współczynnik $h$ wyznaczany jest na podstawie wartości $l_l$ albo $l_p$ w zależności czy wyznaczane jest położenie biodra dla lewej albo prawej strony. Współczynnik $h$ zdefiniowany został według wzoru \eqref{eq:hipEquation:Hcoeficient}.
	...
	Współczynnik $a$ wyznaczony jest jako połowa odległości pomiędzy stawem kości miedniczej a położeniem markera LASI albo RASI, natomiast współczynnik $m$ określa średnicę markerów wykorzystywanych w trakcie śledzenia. Kąty $\theta$ oraz $\beta$ przyjmują stałe wartości odpowiednio $0.5 rad$ oraz $0.314 rad$. W dokumentacji systemu śledzenia Vicon\cite{ViconModelingInstruction} można również znaleźć opisy wyznaczania położenia dla pozostałych stawów.
	%cały ten podrozdział jak dla mnie jest do przepisania, bo przecież pozycję można obliczac zliczeniowo, na podstawie przyspieszeń.!!!
	\subsubsection*{Wyznaczanie położenia stawów w inercyjnych i magnetycznych systemach śledzenia ruchu}
	Inercyjne i magnetyczne systemu śledzenia ruchu wymagają nieco innego podejścia do wyznaczania położenia poszczególnych stawów ponieważ nie posiadamy informacji dotyczących ich położenia w przestrzeni, a jedynie ich orientację ([TODO] i ewentualne przemieszczenie na podstawie akcelerometru. Czy akcelerometr nie jest czujnikiem inercyjnym??!!). Z tego też powodu, systemy śledzenia ruchu wykorzystujące jedynie czujniki inercyjne i magnetyczne nie pozwalają na śledzenie przemieszczania się śledzonej postaci, a jedynie pozę w jakiej sie ona znajduje. W związku z tym korzeń modelu szkieletowego w systemie śledzenia ruchu opartym o czujniki inercyjne i magnetyczne pozostaje nieruchomy. Możliwe jest zdefiniowanie tylko fragmentu modelu szkieletowego zawierającego np. ramię i przedramię prawej ręki którego korzeniem będzie staw barkowy. Wówczas cały model zbudowany będzie z 3 stawów, jednego nieruchomego odpowiadającego stawowi barkowemu i dwóm ruchomym: łokciowym i nadgarstkowym.\\
	%[TODO] z powyższym akapitem się nie zgadzam, bo przecież istnieją inercyjne systemy śledzenia ruchu postaci. To trzeba przepisać.
	...
	Rysunek \ref{fig:literature:imuMarkerPlacementSample} pokazuje przykładowy sposób umieszczenia markerów inercyjnego systemu śledzenia ruchu na powierzchni ręki, tak aby ich położenie możliwie pokrywało się ze środkiem mas poszczególnych segmentów kończyny ([TODO] dopisałem końcówkę, ale nie wiem czy tak jest w rzeczywistości). Inercyjne markery są w stanie dostarczyć jedynie informacje o ich orientacji w przestrzeni ([TODO] a nie przemieszczenia? A co z akcelerometrem?), co można przełożyć na informację dotyczacą obrotu kości, przy której taki marker jest umieszczony. Nie otrzymujemy jednak wprost informacji o położeniu konkretnych stawów. Posługując się przykładowym schematem z rysunku \ref{fig:literature:imuMarkerPlacementSample} przyjmijmy staw barkowy (3) jako korzeń. Chcąc określić położenie stawu łokciowego (4) niezbędna jest informacja o orientacji przestrzennej markera umieszczonego na ramieniu (1) oraz długość kości ramieniowej, która łączy stawy barkowy i łokciowy. Mając te dwie informacje należy dokonać obrotu wektora o długości takiej jak długość kości ramieniowej do orientacji w jakiej znajduje się marker rozważany marker (1). Wówczas punkt początkowy ([TODO] jaki początek, czego poczatek?) reprezentuje staw barkowy i znajduje się w punkcie $(0 , 0 , 0)$ natomiast punkt końcowy reprezentuje staw łokciowy, a jego współrzędne $(X , Y , Z)$ odpowiadają położeniu tego stawu w przestrzeni. Wyznaczenie położenia stawu nadgarstkowego (5) odbywa się analogicznie, przy czym punktem początkowym jest położenie stawu łokciowego (4), a markerem którego orientacja jest brana pod uwagę jest marker umieszczony na przedramieniu (2). W przypadku systemów śledzenia ruchu opartego na markerach inercyjnych, w których położenie stawów nie jest wyznaczane wprost z uzyskanych pomiarów, ale mymaga dodatkowych obliczeń i transformacji geometrycznych, istotne jest zachowanie hierarchii w jakiej zdefiniowany jest wykorzystany model szkieletowy i wykonywanie obliczeń zgodnie z tą hierarchią.\\
	...
	%\cleardoublepage
	%\phantomsection
	%\chapter{Reprezentacja rotacji obiektu w~przestrzeni}\label{chap:orientstionRep}
	...
	\label{sec:orientstionRep:euler}
	...
	%http://nghiaho.com/?page_id=846	http://www.staff.city.ac.uk/~sbbh653/publications/euler.pdf	http://enu.kz/repository/2009/AIAA-2009-6149.pdf
	...
	%https://en.wikipedia.org/wiki/Quaternions_and_spatial_rotation#Advantages_of_quaternion
	...
	%([TODO] znowu sa wprowadzone jakies współrzędne do szkieletu i wydaje mi się, że te współórzędne dla stawów szkieletu są przy kazdym rysunku inne. Warto by to sprawdzić i ujednolicić)
	\begin{equation}
	...
	\begin{figure}  [!htp] 
		\begin{center}
			\subfigure[Stan śledzenia stawów podczas obrotu ciała \emph{(T-pose)} względem kontrolera]{\label{fig:characteristics:kinect:bodyRotationChart}\includegraphics[scale=0.8]{images/kinectRotation.png}}
			\subfigure[Kąt w łokciu prawej ręki podczas obrotu ciała \emph{(T-pose)} względem kontrolera]{\label{fig:characteristics:kinect:kinectRightHandElbowAngle}\includegraphics[scale=0.8]{images/kinectRightHandElbowAngle.png}} 
		\end{center}
			
		\caption{Pomiary zależne od kąta obrotu względem Kinecta [źródło własne]}
	\end{figure}
	
	\subsubsection*{Szacowanie odległości pomiędzy kontrolerem Kinect, a śledzoną postacią}
	...
	Uwzględniając, charakterystyczne dla kontrolera Kinect, przybliżenie szacowania odległości w jakiej znajduje się użytkownik oraz zaprezentowaną na wykresie \ref{fig:characteristics:kinect:distanceAccuracy} należy przyjąć, że obszar pracy kontrolera Kinect przedstawiony na rysunku \ref{fig:characteristics:kinect:range} nie jest jednorodny ze względu na dokładność szacowania Kinecta ([TODO] trochę to poprzednie zdanie jakieś zapętlone). Fakt ten należy uwzględnić w prowadzonych obliczeniach i wykazane niedokładności skorygować. Jest to o tyle istotne, że na podstawie uzyskanych wyników, przestrzeń, w której szacowanie odległości pomiędzy użytkownikiem a Kinectem jest zbliżone do faktycznej odległości, wynosi jedynie $0.3m$ pomiędzy $2m$, a $2.3m$ od kamery.  
	...
	%([TODO] nie ma wprowadzenia co to jest C i jak to się ma do obliczenia siły działającej na układ. Trzeba uzupełnić jak zmiana pojemności wpływa  na siłę w każdej osi)
	\begin{equation}
	...
	%([TODO] może jakieś zatem podsumowanie rozdziału?)
	...
	%------ Poniżej poprawić
	%czy poprawiłeś to co miałeś poprawić??
